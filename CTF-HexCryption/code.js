...
function createHex()
{
    console.log("Creating HEX code...");
    let text = "";
    for(let index in message)
    {
        let symbol=message[index];
        hexCodes[index] = symbolToHex(symbol);
    }
    console.log(hexCodes);
}

function createBlocks(message)
{
    ...
    for(let hex of hexCodes)
    {
        ...
        if(text.length == 6)
        {
            ...
            newDiv.style.backgroundColor = "#"+text;
            //mainBox.innerHTML += "<div class='colorBox' style='background-color: #"+text+";'></div>"
...
{
    var hex = Number(symbol.charCodeAt(0)).toString(16);
    return hex;
}
function translate()
{
    ...
    {
        let rgb = box.style.backgroundColor;
        let nums = rgb.substring(4, rgb.length-1).split(", ");
        for(let num of nums)
        {
            let letter = String.fromCharCode(parseInt(num));
            message+=letter;
        }        
    }
    console.log(message)
